...
APIs use typed capabilities instead of allocating numeric effect IDs.
...
`HostBuilder::capability`, with DTO declarations generated by `wabou-bindgen`.
Direct structured methods are the default; `json_method` is available when an
external or dynamic JSON boundary makes that codec useful. Long-lived Rust
producers use host messages. This keeps
...
- Long-lived framework resources return a stable handle and publish events.
  Application file watchers, processes, and streams use a capability for
