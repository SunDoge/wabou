...
| `ai` | 7.0.77 | Partially supported | `generateText` and `streamText` run in Wabou QuickJS with a mock provider after installing the bundled WHATWG Streams polyfill. The isolated AI SDK Core probe was about 556 KB minified / 141 KB gzip. Real provider generation can use Wabou `fetch`, but real token streaming remains buffered until the Rust fetch bridge exposes a chunked response body. Keep model orchestration in application JavaScript; do not add AI SDK to `@wabou/core`. |
