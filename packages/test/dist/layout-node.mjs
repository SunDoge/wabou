import { assertNoLayoutDiagnostics, parseLayoutSnapshot, siblingCollisionDiagnostics, styleDiagnostics, visibleOverflowDiagnostics } from "./layout.mjs";
...
async function validateLayoutFixtureReport(report, fixtures) {
	for (const result of report.cases) {
		...
		try {
			...
			for (const check of fixture.checks ?? []) assertNoLayoutDiagnostics(check === "visible-overflow" ? visibleOverflowDiagnostics(result.snapshot) : siblingCollisionDiagnostics(result.snapshot));
