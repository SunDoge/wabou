...
export class CodeEditorDocument {
  ...
  setComposition(
    ...
    if (JSON.stringify(next) === JSON.stringify(this.#composition))
      return false;
  ...
  handleKey(event: CodeEditorKey): { handled: boolean; changed: boolean } {
    const key = event.key;
    if (event.primary && ["c", "v"].includes(key.toLowerCase()))
    ...
    if (key === "ArrowLeft" || key === "ArrowRight") {
      if (!event.shift && !range.empty)
        return collapseOrExtend(key === "ArrowLeft" ? range.from : range.to);
      return collapseOrExtend(
        findClusterBreak(this.value, range.head, key === "ArrowRight"),
      );
    }
    ...
    if (key === "Backspace" || key === "Delete") {
      ...
      return {
        ...
        changed:
          from !== to &&
          this.#apply({ changes: { from, to }, selection: { anchor: from } }),
      };
    }
    if (key === "Enter") {
      if (event.readOnly) return { handled: true, changed: false };
      const indentation =
        this.#state.doc.lineAt(range.head).text.match(/^\s*/)?.[0] ?? "";
  ...
  #snapshot(): Snapshot {
    return { value: this.value, ...this.selection };
  }
  ...
  #apply(spec: TransactionSpec, recordHistory = true): boolean {
    ...
    if (
      ...
      transaction.state.selection.eq(this.#state.selection)
    )
      return false;
    ...
    const fragments =
      this.#tree && transaction.docChanged
    ...
    if (this.#language === "json" && transaction.docChanged)
      this.#tree = jsonParser.parse(this.value, fragments);
  ...
  #restore(source: Snapshot[], destination: Snapshot[]): boolean {
    ...
    this.#tree =
      this.#language === "json" ? jsonParser.parse(this.value) : null;
  ...
  #indent(outdent: boolean): boolean {
    ...
    for (let number = startLine.number; number <= endLine.number; number += 1) {
      ...
      if (outdent) {
        const count = line.text.startsWith("\t")
          ? 1
          : (line.text.match(/^ {1,2}/)?.[0].length ?? 0);
  ...
  #offsetByUtf8Bytes(start: number, delta: number): number {
    ...
    while (
      remaining > 0 &&
      (forward ? offset < this.value.length : offset > 0)
    ) {
      const moved = findClusterBreak(this.value, offset, forward);
      if (moved === offset) break;
      const codePoint =
        this.value
          .slice(Math.min(offset, moved), Math.max(offset, moved))
          .codePointAt(0) ?? 0;
      const bytes =
        codePoint <= 0x7f
          ? 1
