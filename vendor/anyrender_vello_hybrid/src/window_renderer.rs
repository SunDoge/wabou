...
impl WindowRenderer for VelloHybridWindowRenderer {
        ...
        spawn_init(async move {
            ...
            let render_surface = SurfaceRenderer::new(
                ...
                SurfaceRendererConfiguration {
                    ...
                    width,
                    height,
                    // Wabou schedules frames from the Winit event loop. FIFO
                    // presentation can block `queue_present` indefinitely on
                    // Wayland when a window is occluded or lives on another
                    // virtual desktop. Prefer mailbox and fall back to an
                    // immediate mode so the UI thread remains responsive.
    ...
    fn render<F: FnOnce(&mut Self::ScenePainter<'_>)>(&mut self, draw_fn: F) {
        ...
        // Presentation must remain asynchronous. Waiting indefinitely here
        // blocks Winit's event loop on Intel Vulkan + Wayland after the first
        // frame, preventing input, timers, and HMR from being processed.
