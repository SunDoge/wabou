...
pub(crate) struct WinitBehaviorTest {
    ...
    window_keys: Vec<legacy_shell::WindowResourceKey>,
}
...
impl legacy_shell::ShellExtension for WinitBehaviorTest {
    fn requires_semantics(&self) -> bool {
        true
    }

    fn initialize(&mut self, wake: legacy_shell::WakeCallback) -> Result<(), String> {
        self.controller
            .connect_native_windows(self.window_keys.iter().copied(), wake);
        Ok(())
    }

    fn poll(&mut self, context: &mut legacy_shell::ExtensionContext<'_>) {
...
struct WinitTestHost<'a, 'b> {
    context: &'a mut legacy_shell::ExtensionContext<'b>,
}
...
impl NativeTestHost for WinitTestHost<'_, '_> {
    ...
    fn hide_window(
        ...
        self.context.hide_window_with_capabilities(
            window_key,
            Some(legacy_shell::window_lifecycle::WindowCapabilities { mutable_visibility }),
        )
    }

    fn show_window(&mut self, window_key: legacy_shell::WindowResourceKey) -> bool {
    ...
    fn window_viewport(&self, window_key: legacy_shell::WindowResourceKey) -> Option<(u32, u32)> {
        self.context
