...
impl ProjectedElement {
    ...
    fn into_native_wrappers(self) -> AnyElement {
        ...
        let spring = self.native_spring;
        let floating_position = self.floating_position;
        let layout_bounds = self.layout_bounds.clone();
        let mut element = if let Some(spring) = spring {
            NativeSpringElement {
                id: format!("wabou-spring-{}-{}", key.hi, key.lo).into(),
                element: Some(self),
                spring,
            }
            .into_any_element()
