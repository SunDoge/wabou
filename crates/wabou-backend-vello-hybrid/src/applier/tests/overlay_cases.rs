...
fn overflow_container_supports_wheel_and_selection_autoscroll() {
    ...
    let response = applier.handle_event(UiEvent::Wheel(shell_api::WheelEvent {
        ...
        delta_x: 0.0,
        delta_y: 50.0,
        delta_mode: shell_api::WheelDeltaMode::Line,
        phase: shell_api::GesturePhase::Changed,
        modifiers: Modifiers::default(),
    }));
    assert!(response.handled);
    ...
    applier.handle_event(UiEvent::Wheel(shell_api::WheelEvent {
        position: Point { x: 10.0, y: 10.0 },
        delta_x: 0.0,
        delta_y: 5.0,
        delta_mode: shell_api::WheelDeltaMode::Pixel,
    ...
    assert!(
        ...
        applier
            ...
            .map(|hit| (
                ...
                vello_shell::scrollbar::thumb(&hit.placed, ScrollAxis::Vertical)
            ))
            .collect::<Vec<_>>()
    );
