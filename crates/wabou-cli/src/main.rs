...
impl DoctorCheck {
    fn new(status: DoctorStatus, name: impl Into<String>, detail: impl Into<String>) -> Self {
        Self {
            status,
            name: name.into(),
            detail: detail.into(),
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Doctor { app } => doctor(&cwd, app.as_deref()),
        Commands::Dev {
            app,
            port,
            devtools,
...
fn doctor_tool(
    ...
    match command_version(program, args) {
        Ok(version) => checks.push(DoctorCheck::new(DoctorStatus::Pass, program, version)),
...
#[cfg(target_os = "linux")]
fn doctor_platform(checks: &mut Vec<DoctorCheck>) {
...
fn doctor_platform(checks: &mut Vec<DoctorCheck>) {
    match command_version("xcode-select", &["-p"]) {
        Ok(path) => checks.push(DoctorCheck::new(
            DoctorStatus::Pass,
...
fn doctor(cwd: &Path, app_path: Option<&Path>) -> Result<()> {
