...
/// Stable failure categories emitted by the native JSON capability adapter.
///
/// The runtime serializes these values and the TypeScript generator consumes
/// [`JsonCapabilityErrorCode::WIRE_VALUES`], keeping both sides of the envelope
/// contract on one definition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum JsonCapabilityErrorCode {
    /// Rust could not deserialize the request DTO.
    InvalidRequest,
    /// The registered application handler returned an error.
    HandlerFailure,
    /// Rust could not serialize the handler's successful response.
    ResponseEncodingFailure,
}

impl JsonCapabilityErrorCode {
    /// Every code that may be emitted by the native adapter.
    pub const WIRE_VALUES: &'static [Self] = &[
        Self::InvalidRequest,
        Self::HandlerFailure,
        Self::ResponseEncodingFailure,
    ];

    /// Return the stable lower-camel-case wire value.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::InvalidRequest => "invalidRequest",
            Self::HandlerFailure => "handlerFailure",
            Self::ResponseEncodingFailure => "responseEncodingFailure",
        }
    }
}
...
impl CapabilityContract {
    ...
    /// Return the JavaScript namespace property.
    pub const fn name(self) -> &'static str {
        self.name
    }

    /// Return the expected native ABI version.
...
/// TypeScript from Rust types. Native FFI authors write the TypeScript spelling
/// next to the Rust implementation and keep the two semantics aligned.
#[cfg(feature = "generate")]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
#[cfg(feature = "generate")]
impl NativeMethod {
    /// Define a synchronous native method with explicit TypeScript arguments
    /// and response type.
    pub const fn sync(
        name: &'static str,
        arguments: &'static [(&'static str, &'static str)],
    ...
    /// Define an asynchronous native method with explicit TypeScript spelling.
...
impl FunctionModule {
    ...
    /// Add an explicitly described native method.
    pub fn method(mut self, method: NativeMethod) -> Self {
        assert_identifier(method.name, "method");
        assert!(
            self.functions
                .iter()
    ...
    /// Add one hand-written TypeScript declaration used by method signatures.
    pub fn declaration(mut self, name: impl Into<String>, declaration: impl Into<String>) -> Self {
        let name = name.into();
        assert_identifier(&name, "declaration");
        merge_declaration(&mut self.declarations, name, declaration.into());
        self
    }
...
impl Bindings {
    ...
    pub fn render(&self) -> String {
        ...
        for capability in sorted_capabilities(&self.capabilities) {
            ...
            output.push_str("    readonly __wabouCapabilityVersion: number;\n");
        ...
        if !self.capabilities.is_empty() {
            output.push_str("type NativeHostCapabilityErrorCode =\n");
            ...
            output.push_str("const nativeCapabilityErrorCodes = new Set<unknown>([\n");
            for code in JsonCapabilityErrorCode::WIRE_VALUES {
                output.push_str(&format!("  \"{}\",\n", code.as_str()));
            }
            output.push_str("]);\n\n");
            ...
            output.push_str(
                r#"export class NativeCapabilityError extends Error {
...
"#,
        ...
        for capability in sorted_capabilities(&self.capabilities) {
            ...
            output.push_str(&format!(
                "  const nativeCapability = nativeHost.{};\n  assertNativeCapability(nativeCapability, \"{}\", {});\n",
                capability.name,
                capability.name,
                capability.version
            ));
            ...
            for method in sorted_methods(&capability.methods) {
                let parameter = method
                    ...
                    .collect::<Vec<_>>()
                    .join(", ");
                let operation = format!("{}.{}", capability.name, method.name);
                let argument = native_arguments(method, &operation);
                let decode = format!(
                    "decodeNativeResult<{}>(await invokeNativeCapability(nativeCapability, \"{}\", \"{}\"{}), \"{}.{}\")",
                    method.response,
                    capability.name,
                    method.name,
...
#[cfg(feature = "generate")]
fn native_arguments(method: &Method, operation: &str) -> String {
    method
        .arguments
        .iter()
        .map(|argument| format!("encodeNativeRequest({}, \"{operation}\")", argument.name))
        .collect::<Vec<_>>()
        .join(", ")
}
...
mod tests {
    ...
    fn renders_dependencies_raw_abi_and_typed_client() {
        ...
        assert!(output.contains(
            "decodeNativeResult<FunctionResponse>(await invokeNativeCapability(nativeCapability, \"workspace\", \"updateFile\""
        ));
        ...
        for code in JsonCapabilityErrorCode::WIRE_VALUES {
            assert!(
                ...
                "native error code must appear in the generated type and validator: {}",
                code.as_str()
            );
        }
        assert!(output.contains("type NativeHostCapabilityErrorCode ="));
        assert!(output.contains("| \"invalidResponse\""));
        assert!(output.contains("| \"incompatibleHost\""));
        assert!(output.contains("| \"invocationFailure\""));
        assert!(output.contains("| \"requestEncodingFailure\""));
        assert!(output.contains("readonly __wabouCapabilityVersion: number"));
        assert!(output.contains("assertNativeCapability(nativeCapability, \"workspace\", 1)"));
    }
    ...
    fn empty_capabilities_still_render_the_runtime_contract_guard() {
        ...
        assert!(output.contains("assertNativeCapability(nativeCapability, \"empty\", 1)"));
    }

    #[test]
    fn native_capability_error_codes_are_unique() {
        let values = JsonCapabilityErrorCode::WIRE_VALUES
            .iter()
            .map(|code| code.as_str())
            .collect::<BTreeSet<_>>();
        assert_eq!(values.len(), JsonCapabilityErrorCode::WIRE_VALUES.len());
    }
    ...
    fn typed_contract_derives_method_and_wire_encoding() {
        ...
        assert!(output.contains("updateFile(request: FunctionRequest): Promise<FunctionResponse>"));
        assert!(output.contains(
            "invokeNativeCapability(nativeCapability, \"workspace\", \"updateFile\", encodeNativeRequest(request, \"workspace.updateFile\"))"
        ));
    }
    ...
    fn flat_function_modules_preserve_synchronous_signatures() {
        let output = FunctionModule::new("NativeFiles")
            .declaration(
                "FunctionRequest",
                "interface FunctionRequest { path: string; }",
            )
            .declaration(
                "FunctionResponse",
                "interface FunctionResponse { accepted: boolean; }",
            )
            ...
            .method(NativeMethod::sync("currentStatus", &[], "FunctionResponse"))
            .render();
