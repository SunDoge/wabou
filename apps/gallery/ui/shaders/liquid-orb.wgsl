...
struct WabouOrbVertexOutput {
  @builtin(position) position: vec4<f32>,
  @location(0) uv: vec2<f32>,
};

@vertex
fn vs_main(@builtin(vertex_index) index: u32) -> WabouOrbVertexOutput {
  var positions = array<vec2<f32>, 3>(
    vec2<f32>(-1.0, -1.0),
    vec2<f32>(3.0, -1.0),
    vec2<f32>(-1.0, 3.0),
  );
  var output: WabouOrbVertexOutput;
  output.position = vec4<f32>(positions[index], 0.0, 1.0);
  let uv = (positions[index] + vec2<f32>(1.0)) * 0.5;
  output.uv = vec2<f32>(uv.x, 1.0 - uv.y);
  return output;
}

@fragment
fn fs_main(input: WabouOrbVertexOutput) -> @location(0) vec4<f32> {
  let color = orbGlassLiquidAnim(input.uv);
  let pixel = vec2<f32>(input.uv.x, 1.0 - input.uv.y) * u.size;
  let normalized = (2.0 * pixel - u.size) / max(min(u.size.x, u.size.y), 1.0);
  let contourRadius = max(u.radius, 0.05)
    * glsContourScale(normalized, u.time * u.speed, u.contourDeform);
  let squareUv = (2.0 * pixel - u.size) / u.size;
  let feather = 2.0 / max(min(u.size.x, u.size.y), 1.0);
  let fitStart = min(mix(contourRadius, 1.0, 0.5), 1.0 - feather);
  let fit = 1.0 - smoothstep(fitStart, 1.0, max(abs(squareUv.x), abs(squareUv.y)));
  return vec4<f32>(color.rgb * fit, color.a * fit);
}
